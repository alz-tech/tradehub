...
const { requireAuth, requireAdmin, mapUser } = require("../../middleware/auth");
...
router.post("/admin/:username/trial", requireAdmin, async (req, res) => {
  const days = parseInt(req.body?.days, 10) || 14;
  const now = Date.now();
  await pool.query(
    `INSERT INTO subscriptions (username, status, trial_start, trial_end, current_period_end, last_payment_ref)
     VALUES ($1,'trial',$2,$3,NULL,NULL)
     ON CONFLICT (username) DO UPDATE SET status='trial', trial_start=$2, trial_end=$3, updated_at=now()`,
    [req.params.username, now, now + days * DAY_MS]
  );
  res.json({ success: true });
});

router.post("/admin/:username/grant", requireAdmin, async (req, res) => {
  ...
  await pool.query(
    `INSERT INTO subscriptions (username, status, current_period_end)
     VALUES ($1,'active',$2)
     ON CONFLICT (username) DO UPDATE SET status='active', current_period_end=$2, updated_at=now()`,
    [req.params.username, base + days * DAY_MS]
  );
  res.json({ success: true });
});

router.post("/admin/:username/end", requireAdmin, async (req, res) => {
  await pool.query("UPDATE subscriptions SET status='expired', updated_at=now() WHERE username = $1", [req.params.username]);
  res.json({ success: true });
});
